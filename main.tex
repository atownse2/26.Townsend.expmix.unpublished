...
In high-energy searches for physics beyond the Standard Model, analysts often model smoothly falling spectra above some threshold $u$ that is typically well above known resonances. This regime is well suited to Extreme Value Theory (EVT), which provides a rigorous description of tail behavior for large $u$. A central EVT result, the Pickands--Balkema--de Haan theorem \cite{Picklands_1975, Balkema_Haan_1974}, states that for a broad class of underlying distributions, exceedances above a sufficiently high threshold converge to a Generalized Pareto Distribution (GPD). The GPD density is given by:
\begin{equation}
