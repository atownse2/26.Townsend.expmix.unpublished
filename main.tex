...
\abstract{
Searches for new physics at the LHC often look for localized excesses on falling background distributions. Several classes of background models have been considered, including polynomials and other parametric families. In this work, we motivate the finite exponential mixture as a flexible semi-parametric class of functions for approximating falling distributions, drawing on results from extreme value theory. Using several published results, we show that the exponential mixture performance is similar to existing methods for both small ($n=5,036$) and large ($n=28,619,185$) datasets. Finally, in simulation studies, we measure the bias and coverage of the exponential mixture and find it to have small bias and best in class coverage.
}
...
In this paper, we consider the finite exponential mixture as an alternative approach to modeling smoothly falling backgrounds, which is motivated as a generalization of the unbounded asymptotic distribution of threshold excesses. Extreme Value Theory (EVT) provides a rigorous statistical framework for describing the behavior of the tails of distributions. Rather than attempting to model the full, complicated mass spectrum from the underlying physics or relying on ad-hoc functional forms, EVT allows us to justify our approach mathematically. In addition to the mathematical motivations, we show that the finite exponential mixture can match the performance of previous methods in datasets that vary widely in size (from $n=5,036$ to $n=28,619,185$). Finally, in pseudo-datasets, we show that, with an appropriate model selection metric, the finite exponential mixture has small bias and exceptional coverage in the bulk. The exponential mixture is flexible enough to generalize to different datasets, while rigid enough to avoid most overfitting.
...
A foundational result in extreme value theory, the Pickands--Balkema-de Haan theorem \cite{Picklands_1975, Balkema_Haan_1974} states that, beyond a sufficiently high threshold, the tails of a broad class of parent distributions converge to a Generalized Pareto Distribution (GPD). This framework is highly attractive for new physics searches, where signal regions typically lie in the extreme kinematic tails past dominant Standard Model resonances. However, in practical applications, the strict assumptions of EVT may not hold across the entire range of interest. Asymptotic convergence can be slow, or the data may simultaneously span two distinct EVT regimes—such as an intermediate heavy-tailed behavior that eventually transitions into a bounded far-tail. Furthermore, experimental constraints may force the selection of a lower threshold $u$ that sits close to unresolved resonance peaks, where these competing tail dynamics overlap. In these complex, pre-asymptotic and mixed regions, standard GPD assumptions break down, requiring a more flexible modeling approach that can accommodate multiple regimes while remaining asymptotically well-behaved.

The GPD density is given by:
\begin{equation}
f(x | u, \sigma, \xi) = \frac{1}{\sigma} \left( 1 + \xi \frac{x - u}{\sigma} \right)^{-\left( \frac{1}{\xi} + 1 \right)}
\end{equation}
where $u$ is the lower threshold, $\sigma$ is the scale parameter, and $\xi$ is the shape parameter. The GPD domains are strictly categorized by the value of $\xi$: for $\xi>0$, the distribution is heavy-tailed and unbounded; for $\xi=0$, it reduces to a simple exponential distribution; and for $\xi<0$, it terminates at a finite upper bound $x<u-\sigma/\xi$. While the GPD is guaranteed to model the density in some asymptotic regime, its rigid parameterization limits its efficacy when modeling broader, non-asymptotic kinematic ranges.

This limitation is particularly acute in high-energy physics analyses that must contend with competing tail behaviors. For instance, a physical spectrum might exhibit a heavy-tailed falloff over an intermediate range before eventually encountering a kinematic or geometric upper bound. A standard GPD is forced to commit to a single regime via $\xi$: choosing $\xi<0$ captures the upper bound but fails to model the heavy-tail behavior as the threshold $u$ is lowered, while choosing $\xi>0$ models the heavy tail but cannot accommodate a finite boundary. To resolve this tension, a model is needed that captures a wider variety of intermediate tail behaviors while preserving the correct asymptotic limits. Crucially, when $\xi>0$, the GPD can be mathematically framed as an exponential-gamma mixture. This structural relationship directly motivates the use of more general exponential mixtures, which provide the necessary flexibility to model pre-asymptotic and intermediate shapes.

Mixture models are a flexible class of models that can be used to approximate a wide variety of distributions. The density of a general single-parameter mixture ($\theta$) can be written as:
\begin{equation}
\label{general_mixture}
f(x) = \int p(\theta) p(x | \theta) d\theta \, ,
\end{equation}
where $p(x|\theta)$ is the base density and $p(\theta)$ is the mixing density. In general, $p(\theta)$ can be any density function defined on the domain of $\theta$. Without assuming a specific functional form for $p(\theta)$, it is commonly estimated non-parametrically, which yields a discrete distribution concentrated at $k$ distinct support points \cite{Kiefer_Wolfowitz, Lindsay}. This discrete estimate of $p(\theta)$ can be written as:
\begin{equation}
\label{sum_approx}
p_k(\vec\theta)=\sum_{i=1}^k w_i \delta(\theta-\theta_i),
\end{equation}
where $\vec\theta$ is the vector of the $k$ supports $\theta_i$, $\delta(\cdot)$ is the Dirac delta distribution and $\sum w_i=1$ to ensure that the resulting mixture is normalized and identifiable. Substituting \ref{sum_approx} into \ref{general_mixture} we find the common form of the finite mixture density:
\begin{equation}
\label{finite_mixture}
f_k(x|\vec\theta) = \sum_{i=1}^k w_i p(x | \theta_i) \, .
\end{equation}

The class of exponential mixtures is equivalent to the class of completely monotonic functions\footnote{A function is completely monotonic if
\begin{equation}
(-1)^n f^{(n)}(x)\geq 0
\end{equation}
for all $n \in \{0,1,2,\ldots\}$, which means that the function is non-negative, non-increasing, convex, etc.} \cite{bernstein}. The class of completely monotonic functions is closed under addition and multiplication \cite{completetly_monotone}. For functions in this class, it is guaranteed that the finite exponential sum converges uniformly \cite{McGlinn_1978}. This class does not include functions like the falling tail of a Normal distribution, for example, though the approximation becomes better if one restricts the domain to be sufficiently far from the mean. In some regime the Normal distribution tail converges to the GPD with $\xi=0$, which is an exponential distribution.

Using eq. \ref{finite_mixture} we can write the finite exponential mixture density as:
\begin{equation}
f_k(x|\vec{\beta}, \vec{w}) = \sum_{i=1}^k w_i e^{-\beta_i x} \, ,
\end{equation}
where $\vec\beta$ are the rate parameters of the exponential distributions with $\beta_i \in [0,\infty)$. To enforce the condition that the weights $\vec{w}$ sum to 1, the parameters are constructed from a set of unconstrained parameters $\vec{v}$ with $k-1$ degrees of freedom using the softmax function:
\begin{equation}
\label{softmax}
...
% Model Selection
The number of components needed to fit a dataset is determined from that dataset. In contrast to Gaussian mixtures or polynomials, exponentials are rigid, minimizing the risk of overfitting statistical fluctuations in the background or localized signals. An exception to this rigidity occurs near the lower threshold where the model is able to add components with high rates and small weights. Because the exponential has a high rate it falls off very fast affecting only the likelihood of the data near the lower threshold. To control this type of overfitting, we impose an upper threshold on the rate parameters which is set to be 50 times the rate of a single exponential estimated by $1/(\mathbb{E}(X)-X_{\min})$. We also use this reference rate to define a normalized set of rate parameters for the exponential rates so that the initialization is robust to different datasets. With this transformation, the rate parameters tend to be between 0.5 and 2.5.
...
% Dijet function
Instead of fitting the entire spectrum at once, ATLAS uses a sliding window fit method which reduces the range of data considered to be within the vicinity of the signal hypothesis being tested. The dijet function used in this analysis is given by:
\begin{equation}
f_D(x) = p_0(1-x)^{p_1}/x^{p_2+p_3\log x},
\end{equation}
where $x=m_{jj}/\sqrt{s}$ for the two jet invariant mass $m_{jj}$ and the center-of-mass energy $\sqrt{s}=13$ TeV. Simpler versions of this function exist, i.e., with $p_3=0$, but as datasets become larger these parameterizations must be extended. For our studies, we do not replicate the sliding window fit method used in this analysis; instead, we fit the entire spectrum at once.

% Binning explanation
The dataset available on \texttt{HEPData} \cite{Maguire_2017} is binned. Datasets in high-energy physics can be very large, causing fits to the original (unbinned) dataset to be time-consuming. In these cases, it is common to bin the data fine enough not to lose information, but wide enough that the fits converge in reasonable times. The bin widths in this case are determined by the dijet invariant mass resolution, which broadens at higher values. The binned likelihood is then given by the product of Poisson probabilities:
\begin{equation}
...
\begin{figure}[h]
    ...
    \caption{Fit of Dijet function and exponential mixture to ATLAS Run 2 Dijet dataset (top) and normalized residuals (bottom).}
    \label{fig:dijet_fit}
\end{figure}
...
The fit of the dijet function and the 4-component exponential mixture to the Dijet dataset is shown in figure \ref{fig:dijet_fit}. Looking at the normalized residuals (Pull), we see that the two functions provide similar estimates throughout the mass range.

% The AIC and BIC scores are shown in Table \ref{tab:dijet_AIC_BIC}. [Weird numbering - why isn't this Table 1?] The exponential mixture provides a better fit in terms of AIC and the dijet function provides a better fit in terms of BIC. 
...
\end{align}
where $x\equiv m_{\gamma\gamma}/\sqrt{s}$, $m_{\gamma\gamma}$ is the invariant mass of two photon system and $\sqrt{s}$ is the center-of-mass energy. Candidate functions are chosen from larger, generic classes of analytic expressions (such as power laws, exponentials, and threshold-shifted polynomials) similar to those used in the dijet function. For each functional family, the lowest-order function within each class that yields a $\chi^2$ goodness-of-fit p-value greater than 5\% is selected.

With the published parameter estimates on \texttt{HEPData} we can see that $f_2$ and $f_3$ are completely monotone functions. The first function $f_1$ has completely monotone components but the term $x^{p_2 \log(x)}$ is not; most departures from complete monotonicity are found at low $x$. The derivatives of function four $f_4$ are more complicated than the other functions, with no guarantee of complete monotonicity, though it is possible for the given published parameter estimates.
...
% Show model selection plot
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{diphoton_model_selection.png}
    \caption{AIC (red dot) and BIC (blue x) scores for exponential mixtures on the CMS high mass diphoton dataset for $k=1,2,3,4$ mixture components.}
...
\begin{figure}[h]
    ...
    \caption{Fit of original functions and exponential mixture (AIC) to CMS high mass diphoton dataset (top) and normalized residuals (bottom). For visualization, the data/density functions are binned.}
...
Though the fit is performed without binning, we bin the dataset for visualization. The fits of all four original functions and the exponential mixture are shown in Figure \ref{fig:diphoton_fit}. In the normalized residuals we see that the estimates are very similar and that the differences are small relative to the uncertainty of the binned data.

% The AIC and BIC scores are shown in Table \ref{tab:diphoton_AIC_BIC} [again, should be table 2.] where $f_2$ has the smallest AIC, followed by the exponential mixture model.
...
\caption{Coverage of estimated density for various functions. Each row represents $T=400$ datasets simulated from the true model with $n=5,036$ entries in each. The 95\% confidence interval is calculated using $B=1000$ bootstraps.}
\label{fig:coverage}
\end{figure}
...
\begin{figure}
    ...
    \caption{Shown is the comparison of the (truth normalized) expected signal yield $S(x)$ for several data generating functions and the exponential mixtures. Each point represents the average over $T=1000$ pseudo-datasets, and the bands represent the 1 standard deviation interval. The bands are only shown for the true model and the exponential mixture chosen by AIC. Each subplot represents a different data-generating model.}
    \label{fig:spurious}
\end{figure}
