...
In high energy searches for new physics, analysts are interested in modeling the tails of falling distributions which are generally well above Standard Model resonances. A foundational result in EVT, the Pickands--Balkema--de Haan theorem \cite{Picklands_1975, Balkema_Haan_1974}, states that for a broad class of underlying probability distributions, the tail beyond a suitably high threshold converges to the Generalized Pareto Distribution (GPD). The GPD density is given by:
\begin{equation}
    f(x | u, \sigma, \xi) = \frac{1}{\sigma} \left( 1 + \xi \frac{x - u}{\sigma} \right)^{-\left( \frac{1}{\xi} + 1 \right)}
\end{equation}
where $u$ is the threshold, $\sigma$ is the scale parameter, and $\xi$ is the shape parameter. The GPD domains are specified by the value of $\xi$: for $\xi > 0$, the distribution is heavy-tailed (Fréchet domain); for $\xi = 0$, it reduces to the exponential distribution (Gumbel domain); and for $\xi < 0$, it has a finite upper bound $x < u - \sigma/\xi$ (Weibull domain). Here we focus on the unbounded cases ($\xi \geq 0$), however in principle there exists upper bounds on the energy spectra due to the finite beam energy. In practice, however, these upper bounds are often far from the observed data and the spectra are well approximated by the unbounded case.


% Introduce Extreme Value Theory
In the search for new physics at high invariant masses, analysts are fundamentally concerned with modeling the extreme tails of falling distributions well above all known Standard Model resonances. Extreme Value Theory (EVT) provides a rigorous statistical framework for describing the asymptotic behavior of these probability tails. Rather than attempting to model the full, complicated mass spectrum from first principles or relying on ad-hoc functional forms, EVT allows us to mathematically formalize the expected shape of the background by focusing solely on events that exceed a sufficiently high threshold.

Suppose we establish a high invariant mass threshold, $u$, and observe events that exceed this value. We are primarily interested in the distribution of the mass excesses, $x - u$, conditional on the invariant mass passing the threshold ($x > u$).  Consequently, the GPD serves as a universal model for the tails of falling distributions.

The GPD is characterized by a scale parameter and a shape parameter, $\xi$, which dictates the severity of the tail. For heavy-tailed distributions, we operate in the regime where $\xi>0$. In this unbounded regime, the GPD possesses a particularly useful mathematical property: it can be expressed exactly as a continuous mixture of exponential distributions. Specifically, it corresponds to an exponential-gamma mixture, where the exponential rate parameter is distributed according to a Gamma distribution. 

Any function that can be constructed as a mixture of exponentials with a positive mixing measure belongs to the class of completely monotone functions. Because the unbounded GPD for $\xi>0$ can be written as an exponential-gamma mixture, it is inherently completely monotone. We can therefore motivate the finite exponential mixture model as a natural and flexible generalization of the GPD. By constructing our background models from a finite exponential mixture, we adopt a basis that spans the space of completely monotone functions. This allows the model to retain the well-motivated asymptotic properties of extreme value distributions, while providing the additional degrees of freedom necessary to fit complex, falling invariant mass spectra over wide analysis ranges.
