...
A foundational result in extreme value theory, the Pickands--Balkema--de Haan theorem \cite{Picklands_1975, Balkema_Haan_1974} states that, in some regime, the tails of a broad class of parent distributions converge to a Generalized Pareto Distribution (GPD).  is In high-energy searches for physics beyond the Standard Model, analysts often model smoothly falling spectra above some sufficiently high threshold $u$. The GPD density is given by:
\begin{equation}
    f(x | u, \sigma, \xi) = \frac{1}{\sigma} \left( 1 + \xi \frac{x - u}{\sigma} \right)^{-\left( \frac{1}{\xi} + 1 \right)}
\end{equation}
where $u$ is the lower threshold, $\sigma$ is the scale parameter, and $\xi$ is the shape parameter. The GPD domains are specified by the value of $\xi$: for $\xi > 0$, the distribution is heavy-tailed; for $\xi = 0$, it reduces to the exponential distribution; and for $\xi < 0$, it has a finite upper bound $x < u - \sigma/\xi$. The GPD will model the density arbitrarily close to the upper bound of the parent distribution, but it may not be a good approximation overall.

Relevant to HEP analyses is the case of a heavy-tailed distribution with a finite upper bound. Close enough to this upper bound, the GPD with $\xi < 0$ will be a good approximation, but as the lower threshold decreases, the GPD will fail to capture the heavy tail behavior of the distribution. Alternatively, if there are no data near the upper bound, the GPD with $\xi>0$ may be a good approximation for the bulk of the data. In the case where both the upper bound and the heavy tail are relevant, the GPD will fail to capture both behaviors, and a more flexible model is needed. If $\xi>0$, the GPD can be represented as an exponential-gamma mixture. This motivates the use of more general exponential mixtures to capture a wider variety of tail behaviors.
...
\end{equation}
where $p(x|\theta)$ is the base density and $p(\theta)$ is the mixing density. In general, $p(\theta)$ can be any density function defined in the domain of $\theta$, although it is commonly estimated non-parametrically as a discrete distribution with $k$ supports. This discrete estimate of $p(\theta)$ can be written as:
\begin{equation}
\label{sum_approx}
...
(-1)^n f^{(n)}(x)\geq 0
\end{equation}
for all $n \in \{0,1,2,\ldots\}$. This means that the function is non-negative, non-increasing, convex, and so on.} \cite{bernstein}. The class of completely monotonic functions is closed under addition and multiplication \cite{completetly_monotone}. For functions in this class, the finite exponential sum is guaranteed to converge uniformly \cite{McGlinn_1978}. This class does not include functions like the falling tail of a Normal distribution, for example, though the approximation becomes better if one restricts the domain to be sufficiently far from the mean. For large enough lower thresholds the Normal distribution tail converges to the GPD with $\xi=0$, which is an exponential distribution.
...
\end{equation}
where $x=m_{jj}/\sqrt{s}$ for the two jet invariant mass $m_{jj}$ and the center-of-mass energy $\sqrt{s}=13$ TeV. We do not replicate the sliding window fit method here; instead, we fit the entire spectrum at once for our comparisons.
...
\end{align}
where $x\equiv m_{\gamma\gamma}/\sqrt{s}$, $m_{\gamma\gamma}$ is the invariant mass of two photon system and $\sqrt{s}$ is the center-of-mass energy. 
