...
Searching for new particles is a core part of high-energy physics, new particles offer us the opportunity to fundamentally extend our understanding of the universe. At the Large Hadron Collider (LHC), these searches can take the form of resonance searches—or "bump hunts"—where high-energy proton collisions are used to produce short-lived, novel states. If a new particle decays into visible final-state products within the detector, its presence can be identified as a statistically significant localized excess, or "bump," in the reconstructed invariant mass. Standard Model (SM) processes can mimic these decays of new particles; however, in the regions of interest their contributions are often non-resonant. Isolating a genuine signal on top of these sometimes large SM contributions requires an accurate description of the SM expectation.
...
\end{equation}
for all $n \in \{0,1,2,\ldots\}$, which means that the function is non-negative, non-increasing, convex, etc.} \cite{bernstein}. The class of completely monotone functions is closed under addition and multiplication. For functions in this class, the finite exponential sum is guaranteed to converge uniformly \cite{McGlinn_1978}.
...
\end{equation}
where $f_t$ is the true density used to generate pseudo-dataset $t$, $\sigma(x)$ is the standard deviation of the estimated densities given the true model, $f$ is the model under 
...
models. For consistent scale, we plot the signal yield $N_s$ divided by the Hessian approximation of its uncertainty $\sigma_s$. Figure~\ref{fig:spurious} shows this quantity for several models and data-generating functions. We see that the median values of the AIC-selected exponential mixture are generally small compared to the variance in the true model and the uncertainty in the exponential mixture largely covers zero. A negative value indicates that the exponential mixture over-predicts the density in that region, the range is restricted to 2000 GeV to prevent underflow that can happen in regions of low density. The spurious signal measured here is consistent with the bias shown in Figure~\ref{fig:bias}: regions where the density is under-predicted correspond to over-prediction of the signal strength, and vice versa.
