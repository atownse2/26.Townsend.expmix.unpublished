...
theory.
Using two published datasets, we show that the exponential mixture performance is comparable 
to existing methods for both small and large datasets.
Finally, in simulation studies, we find that the exponential mixture has small bias and 
consistent coverage in the bulk.
}
...
Searches for new particles are standard in high-energy physics. The Large Hadron Collider (LHC) creates these new particles by colliding beams of protons. If a new particle is produced and decays 
within the detector, with visible decay products, one can estimate the mass of the parent 
particle. A statistically significant excess of decays above the Standard Model (SM) expectation constitutes a discovery. The primary challenge 
in these searches is posed by SM processes that mimic the signal, collectively referred to as ``backgrounds''. Because new physics signals are typically 
rare, they are often obscured by these much more frequent background events, so a proper description of the background model is essential.
...
very flexible to rigid, analysis-specific background models. Flexible models like polynomials 
theoretically have low bias, but may need a large number of parameters to achieve this, and can 
...
such as those used to discover/measure the Higgs boson properties \cite{CMS_Higgs_Discovery, Aad_2014}, 
avoid these issues by only fitting a portion of the observed mass spectra near the Higgs mass, 
and masking the signal window when initializing the background parameters. Since the Higgs mass 
occurs within the bulk of the background, the background model is well constrained by the data 
sidebands; however, other analyses extend their search to the tails, where no sidebands are 
available and better extrapolation techniques are required.

Analysis-specific background models are often more rigid, avoiding some of the drawbacks that 
...
involving photons or electrons, despite the more complicated background composition 
...
In this paper, we consider the finite exponential mixture as an alternative approach to modeling 
...
justify our approach mathematically.
...
bulk. The exponential mixture model is flexible enough to approximate motivated alternatives, yet sufficiently constrained that it cannot produce spurious peaks or other non-monotone structures.
...
\begin{equation}
f(x | u, \sigma, \xi) = \frac{1}{\sigma} \left( 1 + \xi \frac{x - u}{\sigma} 
...
framed as an exponential-gamma mixture. This structural relationship directly motivates the use of more general exponential mixtures, which may provide the necessary flexibility to model 
...
$p(\theta)$ can be any density function defined on the domain of $\theta$. Without assuming a specific functional form for $\pi(\theta)$, it is commonly estimated non-parametrically, yielding a discrete distribution represented by a linear combination of $k$ Dirac delta functions  \cite{Kiefer_Wolfowitz, Lindsay}.
...
The class of exponential mixtures is equivalent to the class of completely monotone functions\footnote{A function is completely monotone if
\begin{equation}
(-1)^n \frac{d^n}{dx^n}f(x)\geq 0
\end{equation}
for all $n \in \{0,1,2,\ldots\}$, which means that the function is non-negative, non-increasing, convex, etc.} \cite{bernstein}. The class of completely monotone functions is closed under addition and multiplication \cite{completetly_monotone}. For functions in this class, the finite exponential sum is guaranteed to converge uniformly \cite{McGlinn_1978}.
This class does not include functions like the falling tail of a Normal distribution, for 
example, though the approximation becomes better if one restricts the domain to be sufficiently far from the mean. In some regime the Normal distribution tail converges to the GPD with 
$\xi=0$, which is an exponential distribution.
...
In the following examples, exponential mixtures are fit by maximum likelihood using the Python interfaces to \texttt{RooFit}~\cite{RooFit} and \texttt{ROOT}~\cite{ROOT}, which are commonly used in high-energy physics.
...
Run~2 data-taking period, spanning from 2015 to 2018 \cite{ATLAS_dijet}.
The publication uses several signal regions defined by the half rapidity separation between the two leading jets $y^*= (y_1 - y_2)/2$, where $y_1$ and $y_2$ are the rapidities of the leading and subleading jets. For this example, we use the signal region defined by $|y^*|<0.6$. 
...
$\sqrt{s}=13$~TeV.
Simpler versions of this function have been used in the past with $p_3=0$, but as datasets grow, these parameterizations must be extended. Because of this concern, ATLAS uses a sliding window fit method \cite{ATLAT_Dijet_sliding_window}, which reduces the fit range to the vicinity of the signal hypothesis being tested. We do not replicate this technique, instead fitting the entire spectrum at once.

The dataset available on \texttt{HEPData} \cite{Maguire_2017} is binned. In high-energy physics, datasets can be very large, making fits to the unbinned data time-consuming. In such cases, it is common practice to bin the data finely enough to retain information, while coarsely enough that the fits converge in reasonable times.
...
entries extending to 10~TeV is appended, as discussed in the previous section.
Since this dataset is large, a higher value of $k$ is expected to be required to model it 
...
\begin{figure}
    ...
    \caption{AIC (red dot) and BIC (blue x) scores for exponential mixtures on the ATLAS dijet 
    dataset for $k=3,4,5,6$ mixture components.}
...
Model & $\chi^2$ & $\chi_\nu^2$ & AIC & BIC \\
\hline
Dijet Function & 96 & 1.3 & 372168582 & 372168628 \\
Exponential Mixture (AIC) & 90 & 1.3 & 372168582 & 372168688 \\
\hline
\end{tabular}
\caption{Goodness-of-fit metrics for the dijet function and exponential mixture fit to the 
...
The goodness-of-fit metrics are shown in Table~\ref{tab:dijet_AIC_BIC}: the exponential mixture 
achieves a slightly smaller $\chi^2$ but uses more parameters.
Figure~\ref{fig:dijet_fit} shows the fit of the dijet function and the 4-component exponential 
mixture to the dijet dataset.
The residuals show that the two functions provide nearly identical estimates 
throughout the mass range.

\subsection{CMS High-Mass Diphoton Spectrum} \label{sec:example2}

The dataset considered in this section is the Run~2 dataset published by CMS in the high-mass 
two-photon final state \cite{CMS_high_mass_diphoton}.
...
two-photon system, and $\sqrt{s}$ is the center-of-mass energy.
These functions are chosen from extendable classes of functions built using power laws, 
exponentials, and threshold-shifted polynomials, most of which are also used in the dijet function. 
For each functional family, the lowest-order function yielding a $\chi^2$ goodness-of-fit 
p-value greater than 5\% was selected.

With the published parameter estimates on \texttt{HEPData} we can see that $f_2$ and $f_3$ are completely 
monotone functions.
The first function $f_1$ has completely monotone components, but the term $x^{p_2 \log(x)}$ is 
not;
most departures from complete monotonicity are found at low $x$. The derivatives of $f_4$ are 
more complicated, with no guarantee of complete monotonicity, though it is possible for the 
...
\begin{figure}
    ...
    high-mass diphoton dataset for $k=1,2,3,4$ mixture components.}
...
\begin{figure}[h]
    ...
    \caption{Fit of the original functions and exponential mixture (AIC) to the CMS high-mass 
    diphoton dataset (top) and normalized residuals (bottom). For visualization, the 
    data and density functions are binned.}
...
Model & $\chi^2$ & $\chi_\nu^2$ & AIC & BIC \\
\hline
$f_1$ & 54.3 & 0.94 & 62070.4 & 62083.5 \\
$f_2$ & 53.5 & 0.92 & 62068.6 & 62081.6 \\
$f_3$ & 54.9 & 0.95 & 62071.5 & 62084.6 \\
$f_4$ & 53.4 & 0.94 & 62070.4 & 62090.0 \\
Exponential Mixture (AIC) & 50.6 & 0.90 & 62069.7 & 62095.8 \\
\hline
\end{tabular}
\caption{Goodness-of-fit metrics for $f_1$, $f_2$, $f_3$, $f_4$, and the exponential mixture 
...
The number of exponential components is determined independently for each pseudo-dataset, and results are shown for both AIC and BIC model selection metric.
Agreement is quantified in terms of bias, coverage, and spurious signal yield.

\subsection*{Bias}
The bias is measured by the average density difference scaled by the standard deviation of the 
density estimated using the true model. This quantity, the relative deviation $D(x)$, is given 
by:
\begin{equation}
    D(x) = \frac{1}{T}\sum_{t=1}^T \frac{f_t(x)-f(x|\hat{\bm{\beta}}_t)}{f_t(x)}
\end{equation}
where $f_t$ is the true density used to generate pseudo-dataset $t$, $f$ is the model under 
test, and $f(x|\hat{\bm{\beta}}_t)$ is the MLE of $f$ for pseudo-dataset $t$. 

Figure~\ref{fig:bias} shows the relative deviation of the exponential mixture with respect to
each of the four data-generating functions in eq.~\eqref{eq:diphoton_fns}, with fits to the remaining three shown for scale. 
These plots are assessed by their minima, maxima, and zero-crossings: a good approximation 
crosses the true value frequently and has small bias.
...
\caption{Percent deviation of the estimated density for various functions. Each row represents 
...
As in the bias study, AIC outperforms BIC. The AIC-selected exponential mixture maintains 
nominal coverage across all data-generating models for $m_{\gamma\gamma}<2000$~GeV. In the tails, the AIC-selected mixture under-covers but nonetheless maintains one of the best 
...
events, and $b(x)$ is the background model. The signal model is a Normal distribution with 
width ranging from 30 to 60~GeV at low and high mass, respectively.
Signal yields are extracted over 1000 pseudo-datasets for signal mass hypotheses from 600 to 
2000~GeV.
...
models.
Since signal yields in the tail are much smaller than those in the bulk, the truth-normalized 
signal yield is used:
\begin{equation}
    S(x) = \frac{1}{T\sigma_{t}}\sum_{i=1}^T N_s^{(i)},
\end{equation}
where $T$ is the number of pseudo-datasets and $\sigma_{t}$ is the population standard deviation 
of $N_s$ under the true background model.

\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{spurious_signal.png}
    \caption{Truth-normalized expected signal yield $S(x)$ for the data-generating functions 
    and the exponential mixtures. Each point represents the mean over $T=1000$ pseudo-datasets; bands show the 
    $\pm1$ standard deviation interval and are displayed only for the true model and the 
    AIC-selected exponential mixture.
    Each subplot corresponds to a different data-generating model.} 
    \label{fig:spurious}
\end{figure}

Figure~\ref{fig:spurious} shows that the variance of the AIC-selected exponential mixture 
generally encompasses that of the true model, with mean differences small relative to the 
variance. The bias in $S(x)$ is consistent with the bias shown in Figure~\ref{fig:bias}: regions where the density is under-predicted correspond to over-prediction of the signal strength, and vice versa.

\section{Discussion} \label{sec:discussion}

The ATLAS dijet and CMS diphoton examples demonstrate that the exponential mixture class is 
broad enough to match the performance of existing methods on datasets that differ substantially 
in size and background complexity.

The dijet function relies on theoretical approximations that take time to develop and may need 
updating as datasets grow. More complex signatures, such as those involving photons or 
electrons, may require models that incorporate more than QCD alone.
There is therefore a need for flexible models with principled constraints that generalize well 
across datasets without requiring extensive theoretical development.
The exponential mixture is intended to fill this role.
More broadly, this approach prioritizes a basis of functions satisfying constraints from 
physics or EVT asymptotic theory over a single analysis-specific functional form.

This work is similar to other methods where researchers define an extendable class of functions 
and use model selection to determine the number of parameters needed to fit a dataset. When 
additional parameters have direct physical meaning, conservative test-based methods such as the 
likelihood ratio test are appropriate. Here, however, the simulation studies show that the 
metric preferring more parameters performs better.
In particular, AIC selects larger mixtures than BIC, and those fits have more zero-crossings in the bias curves, smaller bias, better coverage, and smaller spurious signal.
A natural explanation is that, in contrast to flexible models like polynomials, the exponential 
mixture is sufficiently rigid that additional components primarily reduce underfitting rather 
than introducing overfitting.
In this regime, where overfitting is well controlled, model selection metrics that are more liberal than 
AIC may yield further improvements.

If the true distribution is completely monotone, the finite exponential mixture is guaranteed 
...
The distributions of interest are not guaranteed to be completely monotone. While 
energy-based distributions are expected to be monotone in the absence of SM resonances; complete monotonicity is a stronger condition.
...
In the dijet example, the spectrum must eventually transition from a heavy-tailed regime to a 
finite-energy cutoff as energy increases, which the exponential mixture alone cannot capture. 
In practice, this is only relevant when the data extend close to the kinematic limit. In the 
transition region, the shape is approximately exponential, so a suitably truncated exponential 
...
behavior near the kinematic limit.

One approach would be to extend the dijet function with the exponential mixture: the 
$(1-m_{jj}/\sqrt{s})^{p_1}$ term models the finite-energy cutoff (equivalent to a GPD with 
fixed upper endpoint at $\sqrt{s}=13$~TeV and $\mu=0$), while the power-law term can be generalized with an exponential mixture.
...
remains desirable.
Model selection is also sensitive to the upper threshold of the fit, a consequence of the 
...
Overall, the exponential mixture is a credible, asymptotically motivated class of functions for modeling smoothly falling backgrounds, offering a reusable, constrained alternative to 
analysis-specific functional forms.
The principal open questions concern the optimal choice of complexity, robust optimization, and the incorporation of additional physics-motivated structure when the data require it.
...
without committing to a highly analysis-specific functional form. In the ATLAS dijet and CMS 
high-mass diphoton examples, the exponential mixture gave fits comparable to existing methods despite substantial differences in dataset sizes and background complexity.
In pseudo-dataset studies based on the functions used by the published diphoton search, the AIC-selected exponential mixture has small bias, good coverage in the bulk, and small spurious signal.

These results suggest that the exponential mixture is a promising candidate for modeling falling 
backgrounds in resonant searches.
It is broad enough to adapt to different, smoothly falling spectra while remaining rigid enough 
to avoid most overfitting. 
At the same time, the method has clear limitations. The distributions of interest are not 
guaranteed to be completely monotone, and non-completely-monotone effects such as threshold 
turn-on behavior, finite-energy cutoffs, and other process-specific features may need to be 
modeled separately.
Performance also depends on sensible choices for model selection, fit range, and optimization. 

Overall, the finite exponential mixture is a useful general-purpose basis for falling 
backgrounds, particularly in settings where a reusable model class with a clear statistical motivation is desired. 
Further work on optimization, model selection, and the treatment of non-completely-monotone effects would improve the reliability and generality of this approach. 
