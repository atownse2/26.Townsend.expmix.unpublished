...
$$
where $a$ and $b$ are the rate and shape parameters of the Gamma distribution. The exponential case $\xi=0$ corresponds to a rate distribution concentrated at $\beta=1/\sigma$.
...
}, following the discussion of the mixture-model 
...
\hline
Dijet Function & 96 & 1.3 & 74 & 0.3 & 0 \\
Exponential Mixture (AIC) & 90 & 1.3 & 69 & 0 & 60.4 \\
\hline
\end{tabular}
...
Model & $\chi^2$ & $\chi^2_\nu$ & $\nu$ & $\Delta$AIC & $\Delta$BIC \\
\hline
$f_1$                     & 54.2 & 0.93 & 58 & 1.6 & 1.6 \\
$f_2$                     & 53.5 & 0.92 & 58 & 0.0 & 0.0 \\
$f_3$                     & 54.7 & 0.94 & 58 & 2.5 & 2.5 \\
$f_4$                     & 53.4 & 0.94 & 57 & 1.9 & 8.4 \\
Exponential Mixture (AIC) & 50.6 & 0.90 & 56 & 1.2 & 14.2 \\
\hline
\end{tabular}
...
exponential mixture and in
Table~\ref{tab:diphoton_AIC_BIC} we compare the goodness-of-fit between the four original functions and the two-component exponential mixture. We see that the exponential mixture fit quality is comparable 
to that of the original functions, with the smallest $\chi^2$ and the second smallest AIC.
...
In each case, the data-generating parameters are obtained via maximum likelihood estimation (MLE) on the 
...
density estimated using the data-generating model. This relative bias $B(x)$, is given 
by:
\begin{equation}
    B(x) = \frac{1}{T\sigma(x)}\sum_{t=1}^T (f(x|\hat{\bm{\beta}}_t)-f_{true}(x))
\end{equation}
where $f_{true}$ is the density used to generate pseudo-dataset $t$, $\sigma(x)$ is the standard deviation of the estimated densities when using the data-generating model, $f$ is the model being tested, and $f(x|\hat{\bm{\beta}}_t)$ is the MLE of $f$ for pseudo-dataset $t$. 
...
contained mostly within the one-standard-deviation band of the data-generating model, outperforming the 
...
In the tails, the bias remains within the uncertainty of the data-generating model and decreases at high mass.
...
Separately, alternative basis functions or more specialized parameterizations may offer statistical advantages. If the data-generating distribution is completely monotone, finite exponential mixtures can approximate it increasingly closely as the number of components grows, but this does not imply that they provide the most efficient representation at a finite sample size. In the diphoton example, $f_2$ achieves a slightly smaller AIC than the exponential mixture (see Table~\ref{tab:diphoton_AIC_BIC}), and for the published parameter estimates $f_2$ is itself an exponential mixture. This suggests that useful structure within the mixture class may be captured more efficiently by parameterizations tailored to particular spectra.
